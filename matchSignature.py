...
class signatureMatcher():
    ...
    def matchTemplate(self):
        ...
        # get dimensions to create the template
        top_left = max_loc
    ...
    def match_images(self):
        ...
        #self.res,self.scannedImg=cv2.threshold(self.scannedImg,127,255,cv2.THRESH_BINARY)
        #self.res2,self.storedImg=cv2.threshold(self.storedImg,127,255,cv2.THRESH_BINARY)
        """Detect keypoints"""
    ...
    def display_matches(self, kp_pairs, status=None, H=None):
        ...
        vis = numpy.zeros((max(h1, h2), w1 + w2),
                          numpy.uint8)  # Return a new array of given shape and type, filled with zeros in unit8
        ...
        if len(kp_pairs)<70:
            im=ImageOps.expand(im, border=5, fill='red')
        else:
            im = ImageOps.expand(im, border=5, fill='green')

        imgtk = ImageTk.PhotoImage(image=im)
        return imgtk

    def draw_matches(self, kp_pairs):
        """Draws the matches between the input images """
